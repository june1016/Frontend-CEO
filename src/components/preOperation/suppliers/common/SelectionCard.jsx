...
const SelectionCard = ({ 
  ...
  const handleSupplierChange = (event) => {
    ...
    updateSelection(material.id, supplierId, selection?.paymentOption || "contado");
  };

  const handlePaymentChange = (event) => {
    const paymentOption = event.target.value;
    updateSelection(material.id, selection?.supplierId, paymentOption);
  };

  // Encontrar el proveedor seleccionado
  const selectedSupplier = suppliers.find(s => s.id === selection?.supplierId);
  
  // Encontrar los detalles del material para el proveedor seleccionado
  const materialData = selectedSupplier?.materials.find(m => m.materialId === material.id);

  // Si el Jefe de Producción está contratado, encontrar el mejor proveedor por precio
